package com.ecust.resource.helper;
...
public class MemuHelper {
     ...
     * @return
     */
    public static List<JSONObject> bulid(List<EcustPermission> treeNodes) {
        List<JSONObject> meuns = new ArrayList<>();
        if(treeNodes.size() == 1) {
            EcustPermission topNode = treeNodes.get(0);
            //左侧一级菜单
            List<EcustPermission> oneMeunList = topNode.getChildren();
            for(EcustPermission one :oneMeunList) {
                ...
                List<EcustPermission> twoMeunList = one.getChildren();
                for(EcustPermission two :twoMeunList) {
                    ...
                    List<EcustPermission> threeMeunList = two.getChildren();
                    for(EcustPermission three :threeMeunList) {
